...
###
### source('C:/Kaji/Research/Pikas - New Mexico 2016-2020 (Erik Beever et al)/pika_newMexico_2016through2020/01 New Mexico Pika Occupancy & Abundance Analysis - Process Data.r')
###
### CONTENTS ###
...
# 	### changes from EAB email 2025-12-11
# 	#####################################

# 	# EAB: BAND4, site 15 when all sites ordered alphabetically (ordering is the same for all other sites), last sampled in 2020 : change latest density from NA to 0.0625
# 	# ABS: Already done.

# 	# EAB: BAND5, site 16, last in 2020 : change latest density from NA to 0.05 
# 	# ABS: Already done.

# 	# EAB: Goose7, site 156, last in 2018 : change latest density from 2.5 to NA
# 	# ABS: This was already NA. But should it be? In 2016 (last survey) the density was 2.5.

# 	# EAB: GulchU, site 191, last in 2017 : change latest density from NA to 0.105263
# 	# ABS: Changed as per below:
# 	i <- which(pika$polygonName == 'GulchU')
# 	pika$density2017[i] <- 0.105263
# 	pika$latestDensSurveyYear[i] <- 2017
# 	pika$latestDensity[i] <- 0.105263

# 	# EAB: Redondo26, site 409, last in 2018 : not sure how to deal with this (can we discuss?); density says 0 for that year's survey (but leaving "latestDensity" at NA gives us a sample size of 225, not 226, which is the total of currently pika-occupied sites that we've published), but several other columns say the site was currently occupied; I can't find the original datasheet (hardcopy or scan), so my best guess for density would be 0.1 (but maybe better to keep it at NA....?)
# 	# ABS: Let's keep NA.

# 	# EAB: Redondo28+, site 411, last in 2017 : change latest density from NA to 0.117647
# 	# ABS: Changed as per below:
# 	i <- which(pika$polygonName == 'Redondo28+')
# 	pika$density2017[i] <- 0.117647
# 	pika$latestDensSurveyYear[i] <- 2017
# 	pika$latestDensity[i] <- 0.117647

# 	# EAB: VaCa12, site 533, last in 2018 : change latest density from 0.333 to NA.
# 	# ABS: Already done.
...
# 			# ###############################################################################################
# 			# # NB not using bc so few days >22
