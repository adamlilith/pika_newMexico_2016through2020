...
###
### source('C:/Kaji/Research/Pikas - New Mexico 2016-2020 (Erik Beever et al)/pika_newMexico_2016through2020/03a New Mexico Pika Occupancy & Abundance Analysis - Occupancy with Simple Models - Accounting for Spatial Autocorrelation.r')
### source('E:/Adam/Research/Pikas - New Mexico 2016-2020 (Erik Beever et al)/pika_newMexico_2016through2020/03a New Mexico Pika Occupancy & Abundance Analysis - Occupancy with Simple Models - Accounting for Spatial Autocorrelation.r')
###
### This script is the same as the other of the same name, except that it explicitly includes a spatial kriging term in models to account for spatial autocorrelation. It only analyzes binary occupancy models (no ordinal modeling). The kriging is implemented using gam(y ~ x + s(long, lat, bs = 'gp')), which should "absorb" residual spatial autocorrelation and thus should produce more accurate coefficient estimates for the other predictors. The script produces the same outputs as the other of the same name, but with "Accounting for SAC" in the file names.
###
### CONTENTS ###
### setup ###
###
...
#############
	...
	dirCreate('./Figures & Tables/Occupancy - Simple Models - Accounting for SAC')

say('#################################################')
say('### BINARY simple OCCUPANCY analysis with SAC ###')
say('#################################################')
	...
	formulae <- paste0(formulae, ' + s(longitude, latitude, bs = "gp")')
	...
	###############
		...
		model1 <- gam(presAbs ~ 1, data=pika, family=binomial)
		model2 <- gam(presAbs ~ numHomeRangesScaled, data=pika, family=binomial)
		model3 <- gam(presAbs ~ region, data=pika, family=binomial)
		model4 <- gam(presAbs ~ numHomeRangesScaled + region, data=pika, family=binomial)

		model5 <- gam(presAbs ~ meanDistToClosest4Patches, data=pika, family=binomial)
		model6 <- gam(presAbs ~ numHomeRangesScaled + meanDistToClosest4Patches, data=pika, family=binomial)
		model7 <- gam(presAbs ~ region + meanDistToClosest4Patches, data=pika, family=binomial)
		model8 <- gam(presAbs ~ numHomeRangesScaled + region + meanDistToClosest4Patches, data=pika, family=binomial)
	...
	#######################
		...
		for (formula in formulae) {
			...
			model1 <- gam(form1, data=pika, family=binomial)
			model2 <- gam(form2, data=pika, family=binomial)
			model3 <- gam(form3, data=pika, family=binomial)
			model4 <- gam(form4, data=pika, family=binomial)
			model5 <- gam(form5, data=pika, family=binomial)
			model6 <- gam(form6, data=pika, family=binomial)
			model7 <- gam(form7, data=pika, family=binomial)
			model8 <- gam(form8, data=pika, family=binomial)
	...
	###########
		...
		for (occWindow in c(occWindows_y, NA)) {
			...
			file <- paste0('./Figures & Tables/Occupancy - Simple Models - Accounting for SAC/Occupancy - Simple Binary Models Using All Data - ', nice, ' - Accounting for Spatial Autocorrelation.csv')
	...
	sink('./Figures & Tables/Occupancy - Simple Models - Accounting for SAC/Occupancy - Simple Binary Models Using All Data - 7- & 10-yr Window Coefficient Summary - Accounting for Spatial Autocorrelation.txt')
...
say('##############################################################################################')
	...
	file <- paste0('./Figures & Tables/Occupancy - Simple Models - Accounting for SAC/Occupancy - Simple Binary Models Using All Data - 7 & 10-yr Windows - Accounting for Spatial Autocorrelation.csv')
	...
	sink('./Figures & Tables/Occupancy - Simple Models - Accounting for SAC/Occupancy - Simple Binary Models Using All Data Odds of Switching between Classes - Accounting for Spatial Autocorrelation.txt')
...
say('###############################################################################')
	...
	for (occWindow in c(occWindows_y, NA)) {
		...
		write.csv(imp, paste0('./Figures & Tables/Occupancy - Simple Models - Accounting for SAC/Occupancy - Simple Binary Models Using All Data - ', nice, ' - Var Import - Accounting for Spatial Autocorrelation.csv'), row.names=FALSE)
